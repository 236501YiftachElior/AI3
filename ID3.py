...
class ID3:
    def __init__(self, label_names: list, min_for_pruning=0, target_attribute='diagnosis'):
        ...
        self.tree_root: DecisionNode = None
    ...
    def entropy(rows: np.ndarray, labels: np.ndarray):
        ...
        # ====== YOUR CODE: ======
        for c in counts:
            impurity += -(counts[c] / len(rows)) * math.log2(counts[c] / len(rows))
        # ========================
    ...
    def info_gain(self, left, left_labels, right, right_labels, current_uncertainty):
        ...
        info_gain_value = current_uncertainty - (left_entropy * len(left) + right_entropy * len(right)) / (
                len(left) + len(right))
        # ========================
    ...
    def find_best_split(self, rows, labels):
        ...
        for column_idx, column in enumerate(columns):
            ...
            for i in range(len(sorted_column) - 1):
                value_split = (sorted_column[i] + sorted_column[i + 1]) / 2
    ...
    def build_tree(self, rows, labels):
        ...
        # ========================
        node = DecisionNode(best_question, true_branch, false_branch)
        return node
    ...
    def predict(self, rows):
        ...
        # ====== YOUR CODE: ======
        for row in rows:
            y_pred += [self.predict_sample(row)]
