...
ID = 316089952  # TODO: change it to your personal ID
...
def accuracy(y: np.array, y_pred: np.array):
    ...
    # ====== YOUR CODE: ======
    accuracy_val = (y == y_pred).sum() / len(y)
    # ========================
...
def l2_dist(x1: np.array, x2: np.array):
    ...
    # ====== YOUR CODE: ======
    x1_squared_column = np.expand_dims((x1 ** 2).sum(axis=1), axis=1)
    x2_squared_row = np.expand_dims((x2 ** 2).sum(axis=1), axis=0)
    inner_prod = x1.dot(x2.T)
    dists = np.sqrt(x1_squared_column - 2 * inner_prod + x2_squared_row)
    # ========================
